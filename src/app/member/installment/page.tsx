...
const MemberInstallment = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(`${process.env.NEXT_PUBLIC_API}/bill/${process.env.NEXT_PUBLIC_V}/all/in/unpaid`, {
        params: {
          page: page,
  ...
  return (
    <div>
      ...
      <div className='rounded-[10px] border border-stroke bg-white px-4 pb-3 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card  flex flex-col md:flex-row gap-4 justify-between items-center '>
        ...
        <Link href={`/member/installment/add`}>
          <Button label="เพิ่มรายการ" variant="primary" shape="rounded" className='h-10 md:mt-4' />
        </Link>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5 mt-6   ">
        <div className=' overflow-y-auto'>
          <Table className='  '>
            ...
            <TableBody>
              {data?.map((item, index) => {
                const sumFor10Day = Number(item.total_price) + Number(item.net_installment)
                return (
                  <TableRow key={index} className="border-[#eee] dark:border-dark-3 text-center">
                    ...
                    <TableCell className="min-w-[155px] ">
                      <p className={`text-dark dark:text-white ${item.term_type === 1 ? "bg-yellow-200/50" : "bg-gray-200/50"} rounded-md`}>
                        {item.term_type === 1 ? Number(item.interest_amount).toLocaleString() : Number(item.net_installment).toLocaleString()}  / {item.term_type === 1 ? "วัน " : item.term_type === 2 ? "เดือน" : ""}
                      </p>
                    </TableCell>

                    <TableCell className="min-w-[155px] ">
                      <p className="text-dark dark:text-white">{`${Number(item.paid_amount).toLocaleString()} / ${ Number(item.remaining_amount).toLocaleString()}`} ฿</p>
                    </TableCell>
                    ...
                    <TableCell className="min-w-[200px] ">
                      <Link href={`/member/installment/${item.id}`} className=''>
