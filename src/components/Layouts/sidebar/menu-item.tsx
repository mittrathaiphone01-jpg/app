...
const menuItemListBaseStyles = cva(
  "rounded-lg px-3.5 font-medium text-dark-4 transition-all duration-200 dark:text-dark-6",
  {
    variants: {
      isActive: {
        true: "bg-[rgba(87,80,241,0.07)] text-primary hover:bg-[rgba(87,80,241,0.07)] dark:bg-[#FFFFFF1A] dark:text-white",
        false:
          "hover:bg-gray-100 hover:text-dark-5 hover:dark:bg-[#FFFFFF1A] hover:dark:text-white",
      },
    },
    defaultVariants: {
      isActive: false,
  ...
  if (props.as === "link") {
    return (
      <Link
        ...
        onClick={() => isMobile && toggleSidebar()}
        className={cn(
          menuItemListBaseStyles({
            isActive: props.isActive,
            className: "relative block py-2 font-light",
          }),
          props.className,
        )}
      >
