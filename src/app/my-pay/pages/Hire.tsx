...
const Hire = ({ lineUserId }: propType) => {
    ...
    const handleFachDataBill = async () => {
        try {
            const res = await axios.post(`${process.env.NEXT_PUBLIC_API}/bill/${process.env.NEXT_PUBLIC_V}/all/unpaid/bill/in`, {
    ...
    const handlePay = async (invoice: string, installment_price: number, type: number, bill_id: number, bill_detail_id: number, product_name: string) => {
        const payload = {
            invoice,
            installment_price,
            type,
        ...
        <div className='my-6 mx-auto container  '>
            {loading ? (
                ...
                <Card className='w-full text-center text-2xl text-dark-2 '>
                    <div className=''>
                        <h2 className='text-start '>บิลเช่า</h2>
                    </div>
                    <div>
                        {dataBill.map((item) => (
                            <div key={item.id} className='mt-6'>
                                <Card className='border border-gray-200'>บิล : {item.invoice}</Card>
                                <div className='flex flex-wrap w-full gap-2  justify-center bg-gray-200 rounded-md p-4'>
                                    {item.bill_details.map((itemList, index) => (
                                        <Card className='border border-gray-200' key={itemList.id}>
                                            ...
                                            onClick={() => handlePay(item.invoice, itemList.installment_price, 2, item.id, itemList.id, item.product_name)} />
                                        </Card>
                                    ))}
                                </div>
                            </div>
