...
const ReportRent = ({ page }: proptype) => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(`${process.env.NEXT_PUBLIC_API}/bill/${process.env.NEXT_PUBLIC_V}/all/in`, {
        params: {
          page: myPage,
    ...
    <div>
      ...
      <div className='flex flex-col md:flex-row gap-3'>
        <Card className='w-full '>
          <h2 className='text-2xl flex gap-2 items-center'> <PreviewIcon /> จำนวนที่จ่ายแล้ว</h2>
            ...
            <Table className='  '>
              <TableHeader>
                <TableRow className="border-none bg-[#F7F9FC] dark:bg-dark-2 [&>th]:py-4 [&>th]:text-base [&>th]:text-dark [&>th]:dark:text-white">
                  ...
                  <TableHead className='text-center'>ยอดแลกเงิน</TableHead>
                  <TableHead className='text-center'>ดอกเบี้ยทั้งหมด</TableHead>
              ...
              <TableBody>
                {data?.map((item, index) => (
                  <TableRow key={index} className="border-[#eee] dark:border-dark-3">
                    ...
                    <TableCell className='min-w-[120px] text-center'>
                      <p className="text-dark dark:text-white">{Number(item?.loan_amount).toLocaleString()}</p>
                    </TableCell>

                    <TableCell className='min-w-[120px] text-center'>
                      <p className="text-dark dark:text-white">{item.total_interest_amount}</p>
                    </TableCell >
                    <TableCell className='min-w-[120px] text-center'>
                      <p className="text-dark dark:text-white">{item.fee_amount}</p>
                    </TableCell>
                    <TableCell className='min-w-[120px] text-center'>
