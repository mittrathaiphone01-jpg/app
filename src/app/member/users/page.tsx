...
const MemberUsers = () => {
    ...
    const handleDelete = async (id: number, name: string) => {
        try {
            const alertResult = await AlertConfirm(`ลบข้อมูล - ${name || ""} วัน`, 'ต้องการลบข้อมูลนี้หรือไม่ ?')
            if (!alertResult) return false

            const res = await api.delete(`${process.env.NEXT_PUBLIC_API}/members/${process.env.NEXT_PUBLIC_V}/${id}`)
            console.log(res);
            if (res.status === 204) {
                    ...
                    <Table className='  '>
                        <TableHeader>
                            <TableRow className="border-none bg-[#F7F9FC] dark:bg-dark-2 [&>th]:py-4 [&>th]:text-base [&>th]:text-dark [&>th]:dark:text-white">
                                <TableHead className="min-w-[155px] xl:pl-7.5">ชื่อ</TableHead>
